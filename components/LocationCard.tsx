...
const LocationCard: React.FC<LocationCardProps> = ({ profile }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="mt-8 w-full p-1 bg-zinc-900/50 backdrop-blur-md border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl shadow-black/50"
    >
        <div 
          ref={mapContainerRef}
        ...
        >
            ...
            {/* Emerald Tint Overlay - Colors the white lines/text slightly green */}
            <div className="absolute inset-0 bg-emerald-500/10 mix-blend-overlay z-10 pointer-events-none"></div>
            
            {/* Bottom Gradient for Text Readability */}
            <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-transparent z-20 pointer-events-none"></div>

            {/* Content Content */}
            <div className="absolute bottom-0 left-0 w-full p-6 text-white flex justify-between items-end z-30">
                <div className="pointer-events-auto">
                    <h3 className="text-xl font-bold mb-1 flex items-center gap-2 text-emerald-50">
                        <MapPin size={20} className="text-emerald-500" fill="currentColor" fillOpacity={0.2} /> 
                        Atendimento Presencial
                    </h3>
                    <p className="text-zinc-300 text-sm">{profile.location.address}</p>
                    <p className="text-emerald-400/80 text-[10px] font-bold mt-1 uppercase tracking-widest">{profile.location.city}</p>
                </div>
